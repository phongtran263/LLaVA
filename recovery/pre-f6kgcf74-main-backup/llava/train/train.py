...
from llava.model.llava_arch import parse_cka_channel_drop_indices
...
class ModelArguments:
    ...
    use_pcgrad: bool = field(
        ...
        metadata={
            "help": (
                "Apply exact one-sided PCGrad to direct CKA over trainable "
                "mm_projector parameters. Uses one full-model backward and "
                "supports AMP, gradient accumulation, DDP, and DeepSpeed "
                "ZeRO-2; ZeRO-3 is rejected."
            )
        },
    )
    ...
    cka_loss_final_hidden_weight: Optional[float] = field(default=None, metadata={"help": "Weight for the aggregate raw-vision-to-selected-LLM-hidden CKA loss. Defaults to cka_loss_weight for backward compatibility."})
    cka_loss_channel_keep_ratio: float = field(default=1.0, metadata={"help": "Fraction of remaining hidden feature channels retained for CKA after explicit drops. Uses a deterministic random subset; 1.0 keeps all remaining channels."})
    cka_loss_channel_seed: int = field(default=42, metadata={"help": "Seed for the fixed CKA channel subset. Does not consume the model's global RNG."})
    cka_loss_hidden_channel_drop_indices: Optional[str] = field(default=None, metadata={"help": "Comma-separated zero-based projector-output and selected-LLM-hidden channel indices excluded from CKA, e.g. '0,7,31'. Explicit drop indices are not applied to raw vision channels; keep_ratio may still subsample each feature space independently."})
    # CKA has two terms: projector CKA always follows `cka_loss`, while this
    # option selects independent raw-vision-to-LLM-hidden CKA terms.
    cka_loss_layers: Optional[str] = field(default="final", metadata={"help": "Comma-separated 1-based LLM layer indices and/or 'final'; each selected hidden is compared independently with aligned raw vision-encoder features, e.g. '8,16,24,final'. Use 'all' for every block, 'every4' or 'interval:4' for every k-th block, and '-1' to disable this term. Raw-vision-to-hidden CKA is supported by LLaMA/Qwen; Mistral/MPT use projector CKA only."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    pretrain_projector_pcgrad: bool = field(default=False, metadata={"help": "Apply asymmetric PCGrad to projector CKA during projector-only pretraining."})
...
def train(attn_implementation=None):
    ...
    if training_args.pretrain_projector_pcgrad and model_args.use_pcgrad:
        raise ValueError(
            "--pretrain_projector_pcgrad and --use_pcgrad are mutually exclusive; "
            "enable only one PCGrad implementation."
        )
    ...
    cka_channel_keep_ratio = float(model_args.cka_loss_channel_keep_ratio)
    if not math.isfinite(cka_channel_keep_ratio) or not 0.0 < cka_channel_keep_ratio <= 1.0:
        raise ValueError(
            "cka_loss_channel_keep_ratio must be in (0, 1], got "
            f"{model_args.cka_loss_channel_keep_ratio!r}"
        )
    model.config.cka_loss_channel_keep_ratio = cka_channel_keep_ratio
    model.config.cka_loss_channel_seed = int(model_args.cka_loss_channel_seed)
    hidden_channel_drop_indices = parse_cka_channel_drop_indices(
        model_args.cka_loss_hidden_channel_drop_indices
    )
    hidden_size = getattr(model.config, "hidden_size", None)
    if hidden_size is None:
        hidden_size = getattr(model.config, "d_model", None)
    if hidden_size is not None and hidden_channel_drop_indices:
        hidden_size = int(hidden_size)
        out_of_range_indices = tuple(
            index for index in hidden_channel_drop_indices if index >= hidden_size
        )
        if out_of_range_indices:
            raise ValueError(
                "cka_loss_hidden_channel_drop_indices contains indices "
                f"{out_of_range_indices} outside hidden size {hidden_size}"
            )
        if len(hidden_channel_drop_indices) == hidden_size:
            raise ValueError(
                "cka_loss_hidden_channel_drop_indices cannot remove all "
                f"{hidden_size} hidden channels"
            )
    model.config.cka_loss_hidden_channel_drop_indices = list(
        hidden_channel_drop_indices
    )
    ...
    # "every4" or "interval:4". Each requested hidden state is regularized
    # independently against the aligned raw vision-encoder features.

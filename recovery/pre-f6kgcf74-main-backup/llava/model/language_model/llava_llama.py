...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def _compute_masked_linear_cka_loss(
        ...
        """
        CKA term between selected LLM hidden features and aligned raw vision features.

        The caller passes an attached hidden target and a detached raw vision reference.
        Only positions marked
        ...
        channel_drop_indices = getattr(config, 'cka_loss_hidden_channel_drop_indices', None)
        projected_features = select_cka_feature_channels(
            projected_features,
            keep_ratio=channel_keep_ratio,
            seed=channel_seed,
            salt=2,
            drop_indices=channel_drop_indices,
        )
        layer_hidden_states = select_cka_feature_channels(
            layer_hidden_states,
            keep_ratio=channel_keep_ratio,
            seed=channel_seed,
            salt=3,
    ...
    def _select_vision_feature_subset_from_attention(
        ...
        The LLM-hidden CKA terms below compare every selected hidden state against
        aligned raw vision-encoder features.
        """
    ...
    def forward(
        ...
        self.last_cka_projector_cka_output = None

...
class VSPGradientController:
    ...
    def _process_group_gradients(
        ...
        raw_aux = _sum_gradient_sequences(proj_gradients, final_gradients)
        safe_aux = _sum_gradient_sequences(safe_proj, safe_final)
        raw_aux_norm = _sequence_norm(raw_aux, reference_tensor, process_group=self.process_group)
        safe_aux_norm = _sequence_norm(safe_aux, reference_tensor, process_group=self.process_group)
        ...
        final_sequence = _combine_final_gradients(main_gradients, safe_proj, safe_final)
